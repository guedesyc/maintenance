...
import { parseCatalogWorkbook } from "./_shared/excel.ts";
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (payload.mode === "REPLACE") {
      const { data: currentRows, error: currentRowsError } = await supabase.from("unidades").select("id, nome_normalizado");
      ...
      if (toDeactivate.length > 0) {
        const { error: deactivateError } = await supabase.from("unidades").update({ ativo: false }).in("id", toDeactivate);
    ...
    const { error } = await supabase.from("unidades").upsert(upserts, { onConflict: "nome_normalizado" });
